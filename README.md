...
- 영양 균형, 메뉴 다양성, 원가 효율성, 알레르기 안전성 4개 관점에서 종합 평가하여 점수 산출
...
- Supabase Edge Functions → OpenAI API 호출, 알레르기 회피, 유사 영양가, 예산 조건을 프롬프트에 명시
- 카테고리(밥 / 국 / 반찬)별 맥락을 반영한 3가지 대안 반환, 각 항목에 추천 이유 포함
- 사용자가 선택한 대안으로 원래 메뉴 즉시 교체

### 6. 인쇄 최적화 PDF 내보내기

서버 없이 브라우저에서 PDF를 생성하면서 사이트 CSS와 완전히 격리하는 방식을 적용했습니다.

- iframe 생성 → 독립 HTML/CSS 작성 → html2canvas 렌더링 → jsPDF 삽입 순서로 처리
- 다중 주차를 주차별로 자동 페이지 분할하여 출력
- 콘텐츠 높이 기반 자동 스케일링으로 인쇄 시 레이아웃 유지
- CSV는 BOM(`\uFEFF`) 추가로 Excel에서 한글 인코딩 자동 인식
...
### 4. 공공 API 불안정 → 지수 백오프 & Rate Limiter 구현

**문제**: 공공 API 호출 시 간헐적 실패와 429 Rate Limit 오류가 발생해 식단 보강 작업이 중단되는 경우가 있었습니다.

**개선**: 지수 백오프와 토큰 버킷 Rate Limiter를 구현하여 안정적으로 재시도하도록 처리했습니다.
...
### 5. 다중 주차 날짜 불일치 → 일괄 정규화 로직 구현

**문제**: 주차 추가·삭제·순서 변경·시작일 변경 시 날짜와 주차 번호가 맞지 않는 문제가 발생했습니다.

**개선**: `normalizeWeekPlans()`를 구현해 모든 이벤트에서 날짜와 번호를 일괄 재계산하도록 통일했습니다.

- 배열 순서 기준으로 날짜와 `weekNumber 1..N`을 일괄 재계산
- 5일 운영이어도 주차 간 간격은 7일 유지하여 실제 달력 날짜와 일치
- 삭제·붙여넣기·순서 이동·시작일 변경 모든 이벤트에서 동일한 정규화 로직 적용

---

### 6. 모바일(카카오톡 인앱 브라우저) 로그아웃 오작동
