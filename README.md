...
식단 생성을 두 단계로 분리하여 **속도**와 **정확성**을 동시에 확보했습니다.

- **1단계 (즉시, ~0.8초)**: 검증된 하드코딩 데이터로 식단 구성 후 즉시 화면 표시
- **2단계 (백그라운드)**: 식약처 API → 영양 정보, KAT Online API → 원가 정보 순차 보강
- 초기 설계에서 API를 직접 호출했을 때 생성 시간 5~10초, 키워드 기반 필터링으로 조합이 불안정한 문제를 해결
...
- **협력적 캐싱**: DB 캐시는 전체 사용자 공유. A가 API 호출 후 DB에 저장하면 B는 API 없이 즉시 반환. 로그인 시 LocalStorage → DB 마이그레이션으로 본인 캐시도 공유에 기여
- 영양 보강은 주간→일→끼니 3단계 `Promise.all`로 병렬 처리
- 식약처 API의 100g 기준 영양 정보를 카테고리별 급식소 1인분(밥 210g, 국 250ml, 육류 80g 등)으로 환산
- 칼로리 총량 외에 탄단지 비율(탄수화물 45~65%, 단백질 10~35%, 지방 20~35%)이 권장 범위에 드는지 별도 검증, 심각도에 따라 `warning`/`error` 구분
...
- `normalizeWeekPlans()`: 현재 배열 순서를 주차 순서로 확정 → `baseStartDate + (i × 7일)`로 날짜 재계산 → `weekNumber 1..N` 재부여
