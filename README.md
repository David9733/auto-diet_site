...
- 인원·예산·운영 요일 설정 기반 1~4주 식단 자동 생성
- 같은 날·인접 일자·주간·월간 범위 메뉴 중복 방지
- 식약처 API 연동으로 칼로리·나트륨·단백질 자동 반영
- 농수산식품유통공사 API 연동으로 실시간 도매 시세 기반 원가 계산 및 예산 초과 경고
- 개별 메뉴 재생성·커스텀 입력·특별식(일식·중식·양식) 설정
- 반찬 수 추가·삭제, 운영 안 함 토글
- 식단 저장·불러오기 및 주차 복사·붙여넣기
- CSV·PDF 형식 식단표 내보내기
- AI 영양 균형 분석 및 개선 제안
- 알레르기 항목 포함 메뉴 경고 표시
- Markdown 기반 급식·요리 정보 블로그
