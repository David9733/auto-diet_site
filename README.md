...
"인원과 예산만 입력하면, 한 달치 식단이 완성된다."

급식소 관리자가 매번 수작업으로 식단을 짜고 영양·원가를 따로 계산하는 반복 업무를 없애기 위해 개발한 Next.js 급식소 관리 웹 서비스입니다.<br>
식약처 영양 DB와 한국농수산식품유통공사 실시간 시세 데이터를 연동하여 식단 생성부터 원가 확인까지 자동으로 처리합니다.

| 구분 | 내용 |
|------|------|
| **기존 방식** | 관리자가 직접 메뉴를 선정하고, 영양 정보와 식재료 원가를 별도로 조회·계산 |
| **본 서비스** | 인원·예산·운영 요일을 입력하면 식단을 자동 생성하고, 영양 정보와 실시간 원가를 백그라운드에서 자동 반영 |
...
### 1. 자동 식단 생성
- 급식소 설정(인원, 예산, 운영 요일, 끼니 수, 반찬 수)을 기반으로 1~4주 식단을 자동 생성합니다.
- 밥·국·반찬(육류·채소·김치) 구조의 한식 조합을 구성합니다.

### 2. 메뉴 중복 방지
- 같은 날, 인접 일자(전날·다음날), 주간, 월간(4주) 범위에서 반찬 메뉴 중복을 방지합니다.
- 밥·국·김치는 중복 체크 대상에서 제외하여 자연스러운 식단을 구성합니다.

### 3. 영양 정보 자동 보강
- 식단 생성 직후 백그라운드에서 식약처 API를 호출하여 칼로리·나트륨·단백질 정보를 업데이트합니다.
- API 응답(100g 기준)을 급식소 1인분 기준(메뉴별 차등 적용)으로 변환합니다.

### 4. 원가 정보 자동 보강
- 한국농수산식품유통공사(KAT Online) API를 통해 실시간 도매 시세 기반 식재료 원가를 계산합니다.
- 설정한 예산 대비 원가 초과 시 경고를 표시합니다.

### 5. 식단 편집
- 개별 반찬·밥·국 재생성, 커스텀 메뉴 직접 입력, 특별식(일식·중식·양식) 설정을 지원합니다.
- 반찬 수 추가·삭제, 운영 안 함 토글 기능을 제공합니다.

### 6. 식단 저장·불러오기
- 생성된 식단을 Supabase DB에 저장하고, 히스토리에서 불러올 수 있습니다.
- 주차 식단 복사·붙여넣기로 기존 식단을 재활용할 수 있습니다.

### 7. 내보내기
- 식단표를 CSV 또는 PDF 형식으로 내보냅니다.

### 8. AI 영양 분석
- 생성된 식단의 영양 균형을 분석하고 개선 제안을 제공합니다.

### 9. 알레르기 관리
- 설정한 알레르기 항목이 포함된 메뉴에 경고를 표시합니다.

### 10. 블로그
- Markdown 기반의 급식·요리 관련 정보 블로그를 운영합니다.
...
이 프로젝트는 **AI 코딩 도구를 활용한 개발 workflow**를 설계하고 반복적으로 개선하는 방식으로 진행했습니다.

### 활용한 도구
- **Cursor** — 코드 에디터 내 AI 보조 (컴포넌트 구현, 타입 정의, 리팩토링)
- **Claude CLI** — 구조 설계, 아키텍처 개선 방향 검토, 복잡한 로직 구현

### 개발 workflow

```
1. 기능 요구사항 정의
2. 구현할 기능의 입출력·제약 조건을 프롬프트로 정리
3. Cursor / Claude CLI로 구현 요청
4. 생성된 코드를 실행하여 동작 확인
5. 오류 또는 의도와 다른 동작 시 상황 설명 후 수정 요청
6. 정상 동작 확인 후 코드 구조·성능 개선 요청
7. 최종 동작 확인 후 배포
```

> 취소 토큰 패턴, 하이브리드 아키텍처, 다단계 캐싱 전략 등은 오류 상황을 AI 도구에 설명하고 반복적으로 개선하는 과정에서 구체화된 구조입니다.
